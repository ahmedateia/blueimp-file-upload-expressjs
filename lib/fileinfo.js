...
FileInfo.prototype.initUrls = function() {
    ...
    var that = this;
    var baseUrl = that.options.uploadUrl;
    if (this.error) return;
    if (!this.awsFile) {
        that.url = baseUrl + encodeURIComponent(that.name);
        that.usrName =  encodeURIComponent(that.name);
        ...
        Object.keys(that.options.imageVersions).forEach(function(version) {
            if (_existsSync(that.options.uploadDir + '/' + version + '/' + that.name)) {
                that[version] = baseUrl + version + '/' + encodeURIComponent(that.name);
            } else {
        ...
        that.name = baseUrl + encodeURIComponent(that.name);
        return;
    }
